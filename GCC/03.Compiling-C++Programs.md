...
This section describes how to compile C++ programs using the GNU C++ compiler or ```g++```. Programs can be compiled from a single source file or from multiple source files, and may use system libraries and header files, especially for C++ - the Standard Template Library (STL). ``g++`` is a true C++ compiler it compiles C++ source code directly into assembly language, and is able to provide better support for error reporting, debugging and optimization.
...
Programs using C++ object files must always be linked with g++, in order to supply the appropriate C++ libraries. Attempting to link a C++ object file with the C compiler gcc will cause “undefined reference” errors for C++ standard library functions. For instance,
