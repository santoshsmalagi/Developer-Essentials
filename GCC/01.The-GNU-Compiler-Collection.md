...
* **Platform specific:**
    * The GCC compiler can generate object code for several platforms, and some options only apply when code is being created for a specific platform. 
    * For example, if the output platform is Intel 386, the -fp-ret-in-387 option can be used to specify that floating-point values returned from function calls be stored in the hardware floating-point registers.
