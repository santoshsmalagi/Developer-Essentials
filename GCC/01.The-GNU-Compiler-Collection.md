...
* GCC is written in C with a strong focus on portability, and can compile itself. 
  * It runs on most platforms available today, and can produce output for many types of processors. 
...
*   Adding a new language front-end to GCC enables the use of that language on any architecture, provided that the necessary run-time facilities (such as libraries) are  available.  
